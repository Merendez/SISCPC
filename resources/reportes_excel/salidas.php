...
$result= mysqli_query($link, "select * from salidas");
...
if($row=mysqli_fetch_array($result, MYSQLI_BOTH)){
	date_default_timezone_set('America/Mexico_City');
		...
		$objPHPExcel->getProperties()->setCreator("CONAGUA") //Autor
							 ...
							 ->setTitle("Reporte Excel Reporte Salidas")
							 ->setSubject("Reporte Excel Reporte Salidas")
							 ->setDescription("Reporte Excel Reporte Salidas")
							 ->setKeywords("Reporte Excel Reporte Salidas")
							 ->setCategory("Reporte Excel Reporte Salidas");
...
TEXTO;
		$TituloReporte2="COMICION NACIONAL DEL AGUA\n DEPARTAMENTO DE RECURSOS FINANCIEROS \n REPORTE DE BIENES";
			...
			$titulosColumnas = array('No.', 'idsalidas', 'fecha', 'cantidad',  'idTRABAJADOR', 'CAMB');
...
$CAMB=$row["CAMB"];
		  ...
		  $objPHPExcel->setActiveSheetIndex(0)
        		    ->setCellValue('A'.$i, $i)
		            ->setCellValue('B'.$i, $idsalidas)
        		    ->setCellValue('C'.$i, $fecha)
            		->setCellValue('D'.$i, $cantidad)
        		    ->setCellValue('E'.$i, $idTRABAJADOR)
            		->setCellValue('F'.$i, $CAMB);
