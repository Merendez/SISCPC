...
if($row=mysqli_fetch_array($result, MYSQLI_BOTH)){
	date_default_timezone_set('America/Mexico_City');
		...
		$objPHPExcel->getProperties()->setCreator("CONAGUA") //Autor
							 ...
							 ->setTitle("Reporte Excel Reporte Trabajador")
							 ->setSubject("Reporte Excel Reporte Trabajador")
							 ->setDescription("Reporte Excel Reporte Trabajador")
							 ->setKeywords("Reporte Excel Reporte Trabajador")
							 ->setCategory("Reporte Excel Reporte Trabajador");
...
TEXTO;
		$TituloReporte2="COMICION NACIONAL DEL AGUA\n DEPARTAMENTO DE RECURSOS FINANCIEROS \n REPORTE DE BIENES";
			...
			$titulosColumnas = array('No.', 'idtrabajador', 'nombre', 'apellidoPaterno', 'apellidoMaterno', 'sexo', 'direccion', 'telefono', 'correo', 'Puesto');
...
$Puesto=$row["Puesto"];
		  ...
		  $objPHPExcel->setActiveSheetIndex(0)
        		    ->setCellValue('A'.$i, $i)
		            ->setCellValue('B'.$i, $idtrabajador)
        		    ->setCellValue('C'.$i, $nombre)
            		->setCellValue('D'.$i, $apellidoPaterno)
            		->setCellValue('E'.$i, $apellidoMaterno)
        		    ->setCellValue('F'.$i, $sexo)
            		->setCellValue('G'.$i, $direccion)
            		->setCellValue('H'.$i, $telefono)
        		    ->setCellValue('I'.$i, $correo)
            		->setCellValue('J'.$i, $Puesto);
...
$estiloTituloReporte = array(
       ...
       // $objDrawing2->getShadow()->setDirection(4);
		...
		$objPHPExcel->getActiveSheet()->setTitle('Trabajador');
		...
		header('Content-Disposition: attachment;filename="Trabajadores Registrados.xlsx"');
